angular.module('MyApp')
    .controller('UsersController', ['$scope', '$http', function ($scope, $http) {
        ...
        $scope.removeUser = function(){
            ...
            $http.delete('/api/users/'+$routeParams.id).success(function(data){
                console.log(data);
            });
        ...
        $http.get('/api/users/authentificate').success(function (data) {
            $scope.users = data;
            for (var iter = 0; iter < $scope.users.length; iter++) {
                console.log($scope.users[iter].firstname);
            }

            $scope.authentificate=function () {
                for (var iter = 0; iter < $scope.users.length; iter++) {
                    if(($scope.users[iter].login==$scope.login)&&($scope.users[iter].password==$scope.password))
                    {
                        alert("ok");
                    }
    ...
    .controller('UserCreateController', ['$scope', '$http', '$routeParams', '$location', function ($scope, $http, $routeParams, $location) {
        $http.get('/api/users').success(function (data) {
            $scope.users = data;
        });
        ...
        $scope.addUser = function () {
            var data = {
                ...
                , numberBattery: $scope.numberBattery
                , marque: $scope.marque
